...
from benchkit import projections as proj  # noqa: E402
...
def draw_scaling_panel(ax, campaign, device, target, sizes):
    ...
    for series in campaign.series_on(campaign.timings, device):
        points, projected, fit, estimated = proj.scaling_points(
            campaign, device, target, sizes, series
        )
        if not points:
            continue
                ...
                for c, size in enumerate(sizes):
                    ...
                    for i, por in enumerate(campaign.porosities):
                        ...
                        if np.isfinite(value):
                            ...
                            continue
                        guess = proj.projected_time(campaign, device, name, times, size,
                                                    por, sizes)
    ...
    for device in campaign.devices(campaign.timings):
        ...
        for competitor in campaign.series_on(campaign.timings, device):
            ...
            if competitor in one_size:
                ...
                continue
            drawn = [proj.speedup_matrix(campaign, device, competitor, t, sizes, project=True)
            ...
            for i, (name, values) in enumerate(sorted(measured.items())):
                ...
                for j, size in enumerate(all_sizes):
                    if size in values:
                        ...
                        continue
                    if proj.exhausted_memory(campaign.memory, device=device, series=name,
                                             size=size, porosity=por,
                                             blobiness=campaign.reference_blobiness):
                        # Ran, and did not fit. That is the measurement.
                        oom_slots.append((index[j] + offset, color))
                        continue
                    gib = proj.project_footprint(campaign, device, name, size, por,
                                                 campaign.reference_blobiness, column)
                    if gib is not None:
...
def figure_pareto(campaign):
    ...
    for device in campaign.devices(campaign.timings):
        ...
        for ax, size in zip(axes[0], sizes):
            for name in campaign.series_on(campaign.timings, device):
                sub = fig.select(campaign.timings, device=device, series=name, size=size,
                                 blobiness=campaign.reference_blobiness)
                sub = sub[(sub["rel_error"] > 0) & (sub["time_s"] > 0)]
                if sub.empty:
                    continue
...
def figure_summary(campaign):
    ...
    for row, device in enumerate(devices):
        ...
        matrix, estimated = proj.speedup_matrix(campaign, device, competitor,
                                                fig.PAPER_TARGET, sizes, project=True)
...
def main():
    ...
    if not args.no_publish:
        if publishable(campaign) or args.force_publish:
            publish(campaign, cfg.root.parent)
        else:
            logger.warning(f"not publishing: the results hold none of the "
                           f"{cfg['campaign']['grid']} grid's sizes, so this looks like a "
                           "validation run. Pass --force-publish to override.")
