...
plan = bkdriver.plan_run(
    cfg, args, filename="porespy-cpu.csv", knob_name="tolerance",
    stage=f"bench_porespy --measure={args.measure}",
)
    ...
    for rung, row in enumerate(rows):
        writer.write_row({
            **bkresults.row_prefix(cfg, case, entry, **PREFIX),
            "knob_name": "tolerance", "knob": row.knob, "tau": row.tau, "tau_ref": tau_ref,
            "rel_error": row.rel_error, "time_s": row.time_s, "tau_spread": row.tau_spread,
            "repeats": row.repeats, "stop_reason": row.stop_reason, "note": "",
        })
        logger.info(f"  [{rung + 1:2d}/{len(ladder)}] tol={row.knob:<9.3g} tau={row.tau:.4f} "
...
bkdriver.run_cases(
    cfg, args, plan, prefix=PREFIX, knob_name="tolerance",
