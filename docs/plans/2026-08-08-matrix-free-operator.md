...
2026-08-09 — **Suite gate: PASSED.** Foreground fresh-process `Pkg.test()` at HEAD: **12708 assertions, 0 failures, 0 errors, 4m27s, exit 0**, GPU included — against the campaign floor of 11576, so this campaign adds 1132 assertions and breaks nothing. (An earlier gate run before the two review fixes landed measured 12677; the delta is the dimension-check, ownership and large-`solve(sim)` tests.) Goldens unmoved throughout.
