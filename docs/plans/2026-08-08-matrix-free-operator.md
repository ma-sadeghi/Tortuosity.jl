...
**Benchmarks are the exception and must run in a fresh process.** A warm session has already paid the JIT that a benchmark is supposed to measure, so every number this campaign publishes — apply-vs-CSR, solve timings, certified sizes — must come from a cold `julia` invocation, never from `julia_eval`. The same applies to the foreground `Pkg.test()` release gate and to anything verifying that precompilation itself works.
