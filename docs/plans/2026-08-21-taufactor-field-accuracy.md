...
`benchmarks/investigations/tf_field_vs_tau.py` is the experiment, kept because the tables above are only as good as the method behind them. Run it from `benchmarks/` under pixi, with the taufactor submodule initialised and the image store built. It is a one-off probe rather than part of the campaign, which is why it sits in `investigations/` and not beside the sweep scripts — nothing in `run/` calls it and no result file depends on it.

The raw traces it emits — per-100-iteration error for every case, several hundred kilobytes of JSON — were not kept. Re-running the script regenerates them. Nothing else in the repository was modified by the investigation.
