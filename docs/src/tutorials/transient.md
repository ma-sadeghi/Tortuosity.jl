...
`TransientProblem` accepts three types of boundary conditions for `bc_inlet` and `bc_outlet`:
...
The transient workflow has three steps: create a `TransientProblem`, initialize the state with `init_state`, and advance with `solve!`.
...
sim_ss = TortuositySimulation(img; axis=:x, gpu=false)
...
prob = TransientProblem(img, dt; bc_inlet=1, bc_outlet=0, axis=:x, gpu=false)
sim = init_state(prob)

# Stop when inlet and outlet fluxes converge (near steady state)
solve!(sim, prob, stop_at_delta_flux(0.005, prob))
```

Key parameters of `TransientProblem`:
...
- **`stop_at_delta_flux(delta, prob)`** — stops when inlet and outlet fluxes converge within `delta`.
