...
Measured on the CPU in `Float64` and pooled over five porosities, the unpreconditioned count tracks the edge length at the textbook rate, while the preconditioned count grows only $1.16\times$ from $200^3$ to $600^3$ — a saving of $12.5\times$ in iterations at $200^3$ and $29.6\times$ at $600^3$. At $\varepsilon = 0.2$ it runs 136, 177, then 205 at $1000^3$, against 2721 rising to 7099 unpreconditioned. `solve(sim)` applies it above $10^5$ pore voxels. Its restriction gathers over a fixed adjacency built at setup, pinning the summation order that an atomic scatter would leave to chance.
